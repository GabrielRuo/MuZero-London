...
class Buffer:
    def __init__(
        ...
        self.states = np.zeros(
            (size, d_state), dtype=np.float32
        )  # NOTE: state is not unrolled, just need one initial state for each step
        self.rwds = np.zeros((size, unroll_n_steps), dtype=np.float32)
        self.actions = np.zeros(
            (size, unroll_n_steps), dtype=np.int64
        )  # store action indx
        ...
        self.is_full = (
            False  # needed for when buffer is full to estimate n. of elements
        )
    ...
    def _add(self, buffer, transitions):
        ...
        )  # compute by how many elements the buffer is execeeded
    ...
    def uniform_sample(self, batch_s):
        ...
        num = len(self)
        indx = np.random.choice(np.arange(num), size=batch_s, replace=True).astype(
            np.int64
        )
    ...
    def priority_sample(self, batch_s):
        ...
        num = len(self)
        priorities = (
            self.priorities[:num] ** self._priority_exponent
        )  # take priorities for all elements from buffer
        priorities_probs = priorities / np.sum(
            priorities
        )  # compute a probabilities based on priorities

        indx = np.random.choice(
            np.arange(num), size=batch_s, replace=True, p=priorities_probs
        ).astype(np.int64)
        states, rwds, actions, pi_probs, mc_returns = self._sample(indx)

        # Compute importance weights to scale gradient of each element in the batch based on priority
        weights = (
            (1.0 / self.size) / priorities_probs[indx]
    ...
    def _sample(self, indx):
        ...
        states, rwds = torch.from_numpy(self.states[indx]).to(
            self.dev, dtype=torch.float32
        ), torch.from_numpy(self.rwds[indx]).to(self.dev, dtype=torch.float32)
        actions = torch.from_numpy(self.actions[indx]).to(self.dev, dtype=torch.long)
        pi_probs, mc_returns = torch.from_numpy(self.pi_probs[indx]).to(
            self.dev, dtype=torch.float32
        ), torch.from_numpy(self.mc_returns[indx]).to(self.dev, dtype=torch.float32)
    ...
    def update_priorities(self, indx, new_priorities):
        ...
        if indx is not None:
            assert (
                np.isfinite(new_priorities).all() and (new_priorities > 0.0).any()
